...
/*
 <sui-table-row v-if="false">
      <sui-table-header-cell :style="{background:'none'}"></sui-table-header-cell>
      <sui-table-header-cell :style="{background:'none'}" v-for="(column, idx) in columns" :key="idx" v-if="column.isVisible" :textAlign="getTextAlign(column)"><sui-label pointing="below" basic color="red" v-if="getErrorMsg(column)">{{getErrorMsg(column)}}</sui-label></sui-table-header-cell>
      </sui-table-row>
 */
